...
## Status
Still in progress — the payment method through the application is the main part left to finish.
...
3. **Logs it to a Google Sheet** (`log-booking.js`) — one running spreadsheet of every reservation, so it can all be pulled together at year-end for taxes/analysis (opens in Excel, or loads into Python/pandas or SQL later).
